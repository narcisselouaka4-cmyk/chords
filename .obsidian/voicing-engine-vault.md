...
- **Aucune provenance à l'écran** (décision de Narcisse : « nous on le sait mais les users n'ont pas besoin de savoir ça ») : plus de « Reconstruit d'après les manuels », « Voicing dérivé de … », « (dérivé) » dans le menu des qualités, astérisque du navigateur, « une octave plus bas que VoicingLab », « Main gauche ajoutée » (ni couleur à part sur le mini-clavier), ni « VoicingLab » dans les messages. Les informations restent dans l'état du voicing (`derived`, `rebuilt`, `rebuiltFrom`, `octaveShift`, `addedLH`). Les noms de mouvements corrigés sont gardés.
...
19. Copilote interactif, ton « assistant », Exercices, Sessions, Pédagogie IA (2026-09-25) : marques posées dans le SVG des touches (`keyboard-marks.js` ; rôles, « À remplacer » orange, « Suggérée » en pointillé vert — jamais de rouge « faux ») ; « Qu'en penses-tu ? » sur n'importe quel passage (`live-take.js` : 90 s gardées, passage après une pause de 2,5 s ; `take-review.js` : portrait avec notes exactes, voicing reconnu `voicing-classifier.js`, gamme `scales.js`, suggestions « essaie X à la place de Y » `nearestFitting`) ; pas à pas au clavier (`copilot-steps.js`) ; relecture exacte des sessions (`player.js`) et portrait de session pour le Copilote ; règle « assistant, pas coach » (Narcisse) : suggestions partout, garde-fou `test-suggestion-tone.js` ; Exercices ↔ Copilote (`exercise-context.js`, bouton « Demander au Copilote », outil `play_exercise`, « Qu'en penses-tu ? » comparé à l'exercice) ; Sessions = journal (« Garder dans mes sessions », session `passage`) ; Pédagogie IA : notes du prof (`teacher-notes.js`), V2N rebranché et honnête, transcription du son (`piano-transcriber.py`), registre `copilot-context.js`.
...
- `src/recorder/take-review.js`, `src/recorder/live-take.js`, `src/pedagogie/copilot-steps.js`, `src/ui/keyboard-marks.js` — « Qu'en penses-tu ? », pas à pas et marques du clavier.
- `src/pedagogie/exercise-context.js`, `src/pedagogie/teacher-notes.js`, `src/pedagogie/copilot-context.js` — ce que le Copilote reçoit de l'exercice et du tutoriel.
